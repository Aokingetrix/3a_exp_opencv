...
TEXT_DARK = (50, 50, 50)       # 濃いグレー (基本の文字色)
ACCENT_BLUE = (0, 150, 255)    # Wiiのボタンのような明るい青 (アクセント)
ACCENT_GREEN = (0, 200, 100)   # 成功時の色
ACCENT_RED = (255, 50, 50)     # 失敗・警告時の色
WII_TRANSLUCENT_BG = (245, 245, 255, 170) #半透明の白


def draw_text(surface, text, pos, color = TEXT_DARK, size=24):
    """
...
def create_checkerboard_surface(width, height, color1, color2, tile_size):
    ...
    for y in range(0, height, tile_size):
        for x in range(0, width, tile_size):
            # (x // tile_size) と (y // tile_size) の合計が
            # 偶数か奇数かで色を決定する
...
def render_image(surface, image_path, x, y, w, h, fill_bg=True):
    ...
    try:
        ...
        if fill_bg:
            surface.fill(WII_BACKGROUND, (x, y, w, h)) 
...
# ... FloatingImage クラスの定義の後 ...

class Timer:
    """
    タイマーのアイコンと残り秒数を描画するオブジェクト
    """
    def __init__(self, screen_surface, pos, icon_path, font_path, font_size, text_color, bg_color):
        ...
        try:
            ...
            # アイコンを指定のフォントサイズと大体同じ高さにスケーリング
            aspect_ratio = self.icon_surface.get_width() / self.icon_surface.get_height()
            self.icon_height = font_size
            self.icon_width = int(self.icon_height * aspect_ratio)
            self.icon_surface = pygame.transform.scale(self.icon_surface, (self.icon_width, self.icon_height))
        except Exception as e:
            print(f"タイマーアイコンの読み込みエラー: {e}")
            # エラー時はアイコンなし（四角形などで代替も可）
    ...
    def update(self, remaining_seconds):
        ...
        # テキストを描画したSurfaceを内部に保持
        self.text_surface = self.font.render(text, True, self.text_color)

    def draw(self):
        """
        半透明の角丸背景を含むタイマー全体を描画する
        """
        # 1. 描画する要素のサイズを計算
        text_width = self.text_surface.get_width()
        text_height = self.text_surface.get_height()
        
        # 2. 背景全体のサイズを計算 (アイコン + 隙間 + テキスト + 両端の余白)
        total_width = self.padding + self.icon_width + self.icon_gap + text_width + self.padding
        # (アイコンの高さ or テキストの高さ の大きい方 + 上下の余白)
        total_height = max(self.icon_height, text_height) + (self.padding * 2)
        
        # 3. 半透明描画用のSurfaceを生成 (SRCALPHAが重要)
        try:
            bg_surface = pygame.Surface((total_width, total_height), flags=pygame.SRCALPHA)
            bg_surface.fill((0, 0, 0, 0)) # 念のため完全に透明で初期化
        except ValueError as e:
            print(f"Pygameエラー: Surface作成失敗。画面サイズが不正かも？ {e}")
            return # 描画を中断

        # 4. カプセル型（角丸）の背景を描画
        bg_rect = pygame.Rect(0, 0, total_width, total_height)
        border_radius = total_height // 2 # 高さを半径にするとカプセル型になる
        
        try:
            pygame.draw.rect(bg_surface, self.bg_color, bg_rect, border_radius=border_radius)
        except TypeError:
            # pygame 2.0.0未満で border_radius がない場合のフォールバック
            pygame.draw.rect(bg_surface, self.bg_color, bg_rect) # ただの四角形を描画


        # 5. アイコンとテキストを、そのSurface上に描画 (オフセット位置に)
        
        # アイコンの描画位置 (X=左余白, Y=中央揃え)
        icon_x = self.padding
        icon_y = (total_height - self.icon_height) // 2
        if self.icon_surface:
            bg_surface.blit(self.icon_surface, (icon_x, icon_y))
            
        # テキストの描画位置 (X=アイコンの右側, Y=中央揃え)
...
def main():
    ...
    # ★追加: チェック模様の背景サーフェスを生成
    TILE_SIZE = 40 # チェック模様の1マスのサイズ (お好みで調整)
    background_surface = create_checkerboard_surface(
        SCREEN_WIDTH, SCREEN_HEIGHT, 
        WII_BACKGROUND, WII_CHECK, 
        TILE_SIZE
    )
    ...
    try:
        ...
        font_path = None # Timerクラス側でデフォルトフォントが使われる

    timer_display = Timer(
        screen_surface=screen,
        pos=(CAM_WIDTH // 2 - 70, 10), # 表示位置 (左半分の画面中央上部あたり)
        icon_path="data/clock.png", # ステップ1で準備した画像
        font_path=font_path,
        font_size=40,
        text_color=TEXT_DARK, # (Wii風カラーパレットの変数)
        bg_color = WII_TRANSLUCENT_BG
    )
    ...
    while running:
        ...
        if state == GameState.ROUND_START:
            ...
            if game_manager.round_result_text == "成功":
                result_color = ACCENT_GREEN
            else:
                result_color = ACCENT_RED
            draw_text(screen, game_manager.round_result_text, (120, 250), color = result_color, size = 30)
            ...
            draw_text(screen, f"スコア: {game_manager.score}", (150, 200), color = ACCENT_BLUE,  size=50)
            draw_text(screen, "[S]はじめから", (150, 300), size=30)
            draw_text(screen, "[R]タイトルにもどる", (150, 350), size=30)

        # 共通UI (スコア、ラウンド)
        draw_text(screen, f"スコア: {game_manager.score}", (10, CAM_HEIGHT - 40))
        draw_text(screen, f"ライフ: {game_manager.lives}", (180, CAM_HEIGHT - 40), size = 24)
