...
try:
    ...
    print("emo_recog_gpt.py と game_manager_sur.py が同じディレクトリにあるか確認してください。")
    sys.exit(1)
...
try:
    ...
    import utils    # 汎用関数
    import drawing  # 描画関数
    from ui_elements import FloatingImage, Timer, LifeDisplay # UIクラス
except ImportError as e:
    print(f"エラー: 自作モジュールが見つかりません。({e})")
    print("settings.py, utils.py, drawing.py, ui_elements.py を同じディレクトリに置いてください。")
    sys.exit(1)
